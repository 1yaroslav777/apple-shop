...
const Item = ({ id, title, imageUrl, gigabytes, price, category, rating }) => {
  ...
  const onClickAdd = () => {
    const item = {
      ...
      title,
      imageUrl,
      gigabytes: gigabytes[activeGigs],
      price,
      category,
      count: 0,
    };
  ...
  return (
    <div className="item-block-wrapper">
      <div className="item-block">
        <Link to={`items/${id}`}>
          <img className="item-block__image" src={imageUrl} alt="itemImage" />
        </Link>
        <h4 className="item-block__title">{title}</h4>

        <div className="item-block__selector">
          <ul>
            {gigabytes.map((gigs, id) => (
              <li
                key={id}
                onClick={() => setActiveGigs(id)}
                className={activeGigs === id ? 'active' : ''}>
                {gigs} Gb
              </li>
            ))}
          </ul>
